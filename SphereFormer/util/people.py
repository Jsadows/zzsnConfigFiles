...
Author: Jakub Sadowski (config not the dataset)
...
class People(torch.utils.data.Dataset):
    def __init__(self, 
        data_path, 
        voxel_size=[0.1, 0.1, 0.1], 
        split='train', 
        return_ref=True, 
        label_mapping="util/people.yaml", 
        rotate_aug=True, 
        flip_aug=True, 
        scale_aug=True, 
        ...
        for i_folder in splits:
            self.files += sorted(glob.glob(os.path.join(data_path, "sequences", str(i_folder).zfill(2), 'maps', "*.npy")))
    ...
    def get_single_sample(self, index, vote_idx=0):
        ...
        #raw_data = array.reshape(-1, 3)
        #ones_column = np.ones((raw_data.shape[0], 1))
        #raw_data = np.hstack((raw_data, ones_column))
        raw_data = np.load(file_path).astype(np.float32).reshape(-1, 3)
       # print(raw_data)
        ones_column = np.ones((raw_data.shape[0], 1))
        raw_data = np.hstack((raw_data, ones_column))
        #print(raw_data)

        annotated_data = np.load(file_path.replace('maps', 'labels')[:-3] + 'npy').astype(np.int32).reshape(-1) #np.fromfile(file_path.replace('velodyne', 'labels')[:-3] + 'npy',
