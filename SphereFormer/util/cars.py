...
class Cars(torch.utils.data.Dataset):
    def __init__(self, 
        ...
        for i_folder in splits:
            self.files += sorted(glob.glob(os.path.join(data_path, "sequences", str(i_folder).zfill(2), 'velodyne', "*.pcd")))
    ...
    def get_single_sample(self, index, vote_idx=0):
        ...
        annotated_data = np.load(file_path.replace('velodyne', 'labels')[:-3] + 'npy').astype(np.int32).reshape(-1) #np.fromfile(file_path.replace('velodyne', 'labels')[:-3] + 'npy',
